...
//CRUD
async function registerVehicle(brand, model, image) {
  return new Promise((res,rej) => {
    setTimeout(() => {
      const text = 'INSERT INTO vehicles (brand, model, image) \
        VALUES ($1, $2, $3) \
        RETURNING id ';
...
async function updateVehicle(vehicleId, brand, model, image) {
  return new Promise((res,rej) => {
    setTimeout(() => {
...
router.post("/vehicle/register", async (req, res) => {
  ...
  if(isValid){
    ...
    try{
      const vehicle = await registerVehicle(brand, model, image);
...
router.put("/vehicle/update/:vehicleId", async (req, res) => {
  ...
  if(isValid){
    ...
    try{
      const vehicle = await updateVehicle(vehicleId, brand, model, image);
